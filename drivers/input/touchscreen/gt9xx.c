...
static ssize_t gtp_suspend_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
static DEVICE_ATTR(suspend, S_IWUSR, NULL, gtp_suspend_store);
 ...
 * return: 0-succeed, -1-faileds
 * notice:   Set vdd_ana to global variant because we need to regulator_put this voltage
 *               if we encounter an I2C communication error.
 *               That's becasue the MB has connected another touch IC panel, focaltech maybe.
 */
...
static int gtp_power_switch(struct i2c_client *client, int on)
{
	...
	regulator_set_voltage(vdd_ana, 3000000, 3000000);
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	GTP_INFO("GTP Driver Built@08:00:00, Nov 1 2015");
	...
	init_wr_node(client);
#endif
	sysfs_create_group(&client->dev.kobj, &gtp_attr_group);
