...
static void battery_capacity_check(void)
{
	...
	if (BMT_status.SOC < 95 && metrics_cap_info.bat_95_flag) {
		...
		if (elaps_sec > 0 && elaps_sec < MAX_TIME_INTERVAL) {
			snprintf(buf, sizeof(buf),
					"bq24297:def:time_soc95=1;CT;1,Elaps_Sec=%ld;CT;1:NA", elaps_sec);
	...
	if (metrics_cap_info.last_charger_fault_type != metrics_cap_info.charger_fault_type && metrics_cap_info.bat_fault_flag) {
		metrics_cap_info.bat_fault_flag = false;
		diff = timespec_sub(current_kernel_time(), metrics_cap_info.charge_fault_start_time);
		elaps_sec = diff.tv_sec + diff.tv_nsec / NSEC_PER_SEC;
...
static ssize_t bat_log_write(struct file *filp, const char __user *buff, size_t len, loff_t *data)
{
	...
	if (proc_bat_data == '1')
		pr_notice("enable battery driver log system\n");
	else if (proc_bat_data == '2')
		pr_notice("enable battery driver log system:2\n");
	else
		pr_notice("Disable battery driver log system\n");
...
int init_proc_log(void)
{
	...
	pr_debug("proc_create bat_proc_fops\n");
...
static bool check_top_off_state(void)
{
	...
	if (g_custom_charging_cv == BATTERY_VOLT_04_100000_V) {
		...
		pr_notice("[%s] %d, %d, %d, %lu\r\n", __func__, BMT_status.UI_SOC,
			BMT_status.SOC, top_off_flag, g_custom_plugin_time);
	} else
		top_off_flag = false;
...
static ssize_t current_cmd_write(struct file *file, const char *buffer, size_t count, loff_t *data)
{
	...
	if (sscanf(desc, "%d %d", &cmd_current_unlimited, &cmd_discharging) == 2) {
		...
		pr_notice("  bad argument, echo [enable] > current_cmd\n");
	/*}*/
...
static int current_cmd_read(struct seq_file *m, void *v)
{
	...
	pr_notice("[current_cmd_write] cmd_discharging=%d\n", cmd_discharging);
				     ...
				     loff_t *data)
{
	...
	if (sscanf(desc, "%d %d", &charging_enable, &adjust_power) == 2) {
		pr_notice("[current_cmd_write] adjust_power = %d\n", adjust_power);
		return count;
	} /*else {*/
		pr_notice("  bad argument, echo [enable] > current_cmd\n");
	/*}*/
...
static int mt_batteryNotify_probe(struct platform_device *dev)
{
	...
	if (!battery_dir) {
		...
		pr_debug("proc_create discharging_cmd_proc_fops\n");
